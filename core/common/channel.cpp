    ...
    while (ch->thread.active())
    {
        ...
        if (ch->sourceHost.host.ip)
        {
            ...
            if (ch->sourceHost.tracker)
                peercast::notifyMessage(ServMgr::NT_PEERCAST, "ƒ`ƒƒƒ“ƒlƒ‹ "+chName(ch->info)+" ‚ðƒgƒ‰ƒbƒJ[‚É–â‚¢‡‚í‚¹‚Ä‚¢‚Ü‚·...");
...
void Channel::updateInfo(const ChanInfo &newInfo)
{
    ...
    {
#ifdef WIN32
        // UTF8 ‚©‚à’m‚ê‚È‚¢•¶Žš—ñ‚ð Shift_JIS ‚É•ÏŠ·‚µ‚½‚¢B
        String newName = info.name;
        newName.convertTo(String::T_SJIS);
        ...
        peercast::notifyMessage(ServMgr::NT_PEERCAST, newName.str() + "u" + newComment.str() + "v");
#else
        // Shift_JIS ‚©‚à’m‚ê‚È‚¢•¶Žš—ñ‚ð UTF8 ‚É•ÏŠ·‚µ‚½‚¢B
        String newComment = info.comment;
    ...
    try
    {
        while (thread.active() && !peercastInst->isQuitting)
        {
            ...
            if (checkBump())
            {
                ...
                peercast::notifyMessage(ServMgr::NT_PEERCAST, "ƒ`ƒƒƒ“ƒlƒ‹ "+chName(info)+" ‚ðƒoƒ“ƒv‚µ‚Ü‚µ‚½B");
                error = -1;
                break;
            }
                ...
                if (rawData.writePos > 0)
                {
                    ...
                    }else
                    {
                        if (!isReceiving())
                        {
                            ...
                            String newName = info.name;
                            newName.convertTo(String::T_SJIS);
                            peercast::notifyMessage(ServMgr::NT_PEERCAST, newName.str() + "‚ðŽóM’†‚Å‚·B");
#else
                            peercast::notifyMessage(ServMgr::NT_PEERCAST, info.name.str() + "‚ðŽóM’†‚Å‚·B");
#endif
                        }
