...
void Servent::invokeCGIScript(HTTP &http, const char* fn)
{
    ...
    // Windows Ç≈ Ruby Ç™ñºëOà¯Ç´ÇÇ∑ÇÈÇÃÇ…ïKóvÇ»ÇÃÇ≈ SYSTEMROOT Çí Ç∑ÅB
    if (env_val)
...
void Servent::handshakeGET(HTTP &http)
{
    ...
    {
        // PeerCastStation Ç™ "/" Ç "/html/index.html" Ç… 301 Moved
        // Ç≈ÉäÉ_ÉCÉåÉNÉgÇ∑ÇÈÇÃÇ≈ÅAÉuÉâÉEÉUÇ…ÇÊÇ¡ÇƒÇÕñ≥ä˙å¿Ç…ÉLÉÉÉbÉVÉÖÇ≥ÇÍÇÈÅB
        // "/" Ç…çƒÉäÉ_ÉCÉåÉNÉgÇµÇƒÉLÉÉÉbÉVÉÖÇñ≥å¯âªÇ∑ÇÈÅB
    ...
    {
        // ShoutCast ÉgÉâÉbÉNèÓïÒçXêVópÉGÉìÉhÉ|ÉCÉìÉg
    ...
    {
        // html Ç∆ public ÇÃã§óLÉAÉZÉbÉgÅB
...
// -----------------------------------
// ÉäÉåÅ[ê⁄ë±ÅAÇ†ÇÈÇ¢ÇÕÉ_ÉCÉåÉNÉgê⁄ë±Ç… str Ç≈éwíËÇ≥ÇÍÇΩÉ`ÉÉÉìÉlÉãÇÃÉX
// ÉgÉäÅ[ÉÄÇó¨Ç∑ÅBrelay Ç™ true Ç»ÇÁÇŒÅAÉ`ÉÉÉìÉlÉãÇ™ñ≥Ç©Ç¡ÇΩÇËéÛêMíÜ
// Ç≈Ç»Ç≠ÇƒÇ‡ÅAÉ`ÉÉÉìÉlÉãÇéÛêMíÜÇÃèÛë‘Ç…ÇµÇÊÇ§Ç∆Ç∑ÇÈÅB
...
void Servent::CMD_apply(char *cmd, HTTP& http, HTML& html, String& jumpStr)
{
    ...
    while (cp=nextCGIarg(cp, curr, arg))
    {
        ...
        }else if (strcmp(curr, "expire") == 0)
        {
            ...
            servMgr->logLevel = atoi(arg); // „Åì„Çå„Éê„É™„Éá„Éº„Éà„Åó„Åü„Åª„ÅÜ„ÅåËâØ„Åè„Å™„ÅÑÔºü
    ...
    peercastInst->saveSettings();
    peercast::notifyMessage(ServMgr::NT_PEERCAST, "ê›íËÇï€ë∂ÇµÇ‹ÇµÇΩÅB");
...
void Servent::CMD_fetch(char *cmd, HTTP& http, HTML& html, String& jumpStr)
{
    ...
    // id Ç™ÉZÉbÉgÇ≥ÇÍÇƒÇ¢Ç»Ç¢É`ÉÉÉìÉlÉãÇ™Ç†ÇÈÇ∆Ç¢ÇÎÇ¢ÇÎÇ‹Ç∏Ç¢ÇÃÇ≈ÅAéñ
    // ëOÇ…ê›íËÇµÇƒÇ©ÇÁìoò^Ç∑ÇÈÅB
...
// ÉTÅ[ÉoÉìÉgÇí‚é~Ç∑ÇÈã@î\Çí«â¡ÇµÇΩÇ¢éûÇ…ñÇ…óßÇ¬Ç©Ç‡ÅB
#if 0
...
void Servent::handshakeWMHTTPPush(HTTP& http, const std::string& path)
{
    ...
    // ÉGÉìÉRÅ[É_Å[ÇÃê›íËóvãÅÇì«ÇﬁÅB0 ÉoÉCÉgÇÃãÛÇÃê›íËóvãÅÇ‡çáñ@ÅB
    ...
    // ÇÌÇ©Ç¡ÇΩÇ”ÇËÇÇ∑ÇÈ
    http.writeLine("HTTP/1.1 204 No Content"); // Ç±ÇÍÇ¡ÇƒÇøÇ·ÇÒÇ∆ CRLF èoÇÈÅH
    ...
    // User-Agent ÉwÉbÉ_Å[Ç™Ç†ÇÍÇŒ agent ÇÉZÉbÉg
...
ChanInfo Servent::createChannelInfo(GnuID broadcastID, const String& broadcastMsg, cgi::Query& query, const std::string& contentType)
{
    ...
    // type Ç™ãÛÅAÇ†ÇÈÇ¢ÇÕîFéØÇ≈Ç´Ç»Ç¢èÍçáÇÕÉäÉNÉGÉXÉgÇÃ Content-Type Ç©ÇÁé©ìÆîªï Ç∑ÇÈ
...
void Servent::handshakeHTTPPush(const std::string& args)
{
    ...
    // Ç±Ç±Ç≈ÉVÉÉÉbÉgÉ_ÉEÉìë“ÇΩÇ»Ç≠ÇƒÇ¢Ç¢ÇÃÅH
