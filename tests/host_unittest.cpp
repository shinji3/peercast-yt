...
TEST(HostTest, loopbackIP)
{
    ...
    // 127 ‚Ån‚Ü‚éƒNƒ‰ƒXA‚Ìƒlƒbƒgƒ[ƒN‘S•”‚ªƒ‹[ƒvƒoƒbƒN‚Æ‚µ‚Ä‹@”\‚·
    // ‚é‚ªAloopbackIP ‚Í 127.0.0.1 ˆÈŠO‚É‚Í FALSE ‚ğ•Ô‚·B
...
// 128 ãƒã‚¤ãƒˆã®ãƒ›ã‚¹ãƒˆåã§å†…éƒ¨ãƒãƒƒãƒ•ã‚¡ãƒ¼ãŒ NULçµ‚ç«¯ã•ã‚Œãªããªã‚‹ãƒã‚°ã‚’ç™º
// ç¾ã•ã›ã‚‹ãƒ†ã‚¹ãƒˆã€‚valgrind ãªã©ã§æ¤œçŸ¥ã›ã‚ˆã€‚
